...
### 正则扫描
```
譬如 /news/123  你会去数据查 123的数据 对吗？

那么 我弄几十万个 12312312312312 这种ID

你是不是要去数据查 几十万次select

这个难道 不要做做处理？

就是在路由那做手脚。 收到ID后，要做判断的。 系统会定期更新个 目前ID的最大值

然后预估一个每天的增长量

这样ID得到后 会去扫正则（就是自动根据最大值生成的一个正则）。 如果不在范围内 则返回404
...
2. 如果未命中强缓存，则浏览器会将资源加载请求发送到服务器。服务器来判断浏览器本地缓存是否失效。若可以使用，则服务器

   并不会返回资源信息，浏览器继续从缓存加载资源。
